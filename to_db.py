...
profarea = []       # Прфессиональная сфера
...
for fl in os.listdir(r'C:\Users\aaznu\Works_from_hh\from_hh\docs\vacancies/'):
    ...
    try:
        ...
        for prof in json_obj['specializations']:
            profarea.append(prof['profarea_name'])
            specializations.append(prof['name'])
...
df = pd.DataFrame({'id': ids, 'vacancy': skills_vac, 'skill': skills_name})
